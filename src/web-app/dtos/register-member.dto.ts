import { ArrayMinSize, ArrayNotEmpty, IsArray, IsBoolean, IsEmail, IsNotEmpty, IsString, MaxLength, minLength, MinLength, Validate } from "class-validator";
...
export class RegisterUserDto {
  ...
  @Transform(({ value }) => {
    if (typeof value == 'object' || value === 'true' || value === true || value === '1' || value === 'on') return 'true';
  ...
  @IsArray()
  @ArrayNotEmpty({ message: 'You need to provide at least one valid permission for the registration fo the new member' })
  @ArrayMinSize(1)
  @IsString({ each: true })
